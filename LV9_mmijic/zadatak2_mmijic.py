...
# Optimizacija performansi učitavanja
train_ds = train_ds.prefetch(buffer_size=tf.data.AUTOTUNE)
validation_ds = validation_ds.prefetch(buffer_size=tf.data.AUTOTUNE)
test_ds = test_ds.prefetch(buffer_size=tf.data.AUTOTUNE)

# 2. IZRADA KONVOLUCIJSKE MREŽE
model = keras.Sequential()

# Sloj za skaliranje ulaznih piksela sa [0, 255] na [0, 1]
model.add(layers.Rescaling(1./255, input_shape=(48, 48, 3)))

# Ponavljanje bloka 3 puta s brojem filtara: 32, 64, 128
filteri = [32, 64, 128]
for f in filteri:
...
# Ispis strukture modela u terminalu
model.summary()
...
# 3. DEFINIRANJE CALLBACK FUNKCIJA
callbacks = [
...
# 5. EVALUACIJA NA TESTNOM SKUPU S NAJBOLJIM MODELOM
...
# Dohvaćanje stvarnih i predviđenih klasa
y_true = []
...
# Ispis tekstualnih izvještaja u terminal
print("\nMatrica zabune:")
...
# 6. VIZUALIZACIJA / CRTANJE MATRICE ZABUNE (Novo dodano)
print("\nOtvaram grafički prikaz matrice zabune...")
plt.figure(figsize=(15, 12))

# Crtanje toplinske mape (heatmap) bez ispisa brojeva u ćelijama radi preglednosti (43 klase)
...
# Ova naredba otvara prozor s grafičkim crtežom
plt.show()